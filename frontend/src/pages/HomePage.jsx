...
const HomePage = () => {
  ...
  const handleAddTodo = async () => {
    if (!text) {
        toast("Please write a todo then click add button!", {
          icon: "⚠️",
          style: {
            borderRadius: "10px",
            background: "#333",
  ...
  const handleDeleteTodos = async () => {
    if (selectedTodos.length === 0) {
        toast.error("Please select the Todo to delete", {id:"delete"})
        return;
    }
  ...
  return (
    <div className="max-w-lg mx-auto mt-10">
      <div className="flex items-center space-x-2">
        ...
        />
        <button
          className="bg-blue-500 hover:bg-blue-800 text-white px-4 rounded-lg h-9"
